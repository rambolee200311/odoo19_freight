...
class FreightStatementWizard(models.TransientModel):
    ...
    def create(self, vals_list):
        ...
        for rec in records:
            if rec.shipment_id:
                ...
                # Always rebuild rows server-side so the checkbox list is
                # backed by real freight.service records. New rows are not
                # selected by default.
    ...
    def write(self, vals):
        ...
        if 'line_ids' in vals:
            ...
            if codes & {2, 3, 5, 6}:
                # Delete/clear commands are not a user action in this wizard;
                # restore the authoritative eligible set.
                ...
                return res
            if codes & {0}:
                # Web client rebuilds rows as create commands. Keep the last
                # confirmed server-side selection and apply any explicit
                # (1,...) select updates, then align rows to the authoritative
                # eligible set.
                select_map = {
                ...
                for cmd in vals['line_ids']:
                    if cmd[0] == 1 and cmd[2].get('select') is not None:
                        ...
                        if line.service_id:
                            ...
                            select_map[line.service_id.id] = bool(cmd[2]['select'])
                res = super().write(vals)
                for rec in self:
                    if rec.shipment_id:
                        eligible = rec._eligible_services_for(
    ...
    def _onchange_shipment_id(self):
        ...
        self._rebuild_lines(eligible, False, select_map=select_map)
    ...
    def _onchange_customer_id(self):
        ...
        self._rebuild_lines(
            eligible, self.customer_id, select_map=select_map)
